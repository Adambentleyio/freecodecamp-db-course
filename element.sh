...
# If input is a number
  ...
  GET_SYMBOL=$($PSQL "select symbol from elements where atomic_number = $ATOMIC_NUMBER")
    if [[ -z $GET_SYMBOL ]]
      then
        ...
        GET_TYPE=$($PSQL "select types.type from types inner join properties on types.type_id = properties.type_id where atomic_number = $ATOMIC_NUMBER")
  ...
  if [[ $GET_INPUT =~ ^[A-Za-z]{1,2}$ ]] ; then
  GET_SYMBOL=$GET_INPUT
  # get the atomic number
  ATOMIC_NUMBER=$($PSQL "Select atomic_number from elements where symbol = '$GET_SYMBOL' LIMIT 1")
  if [[ -z $ATOMIC_NUMBER ]]
  then
  echo "I could not find that element in the database."
  else 
  ...
  GET_BOILING_POINT=$($PSQL "select boiling_point_celsius from properties where atomic_number = $ATOMIC_NUMBER")
  GET_MODIFIED
  echo "The element with atomic number $GET_ATOMIC_NUMBER_MODIFIED is $GET_NAME_MODIFIED ($(echo $GET_SYMBOL_MODIFIED)). It's a $GET_TYPE_MODIFIED, with a mass of $GET_ATOMIC_MASS_MODIFIED amu. $GET_NAME_MODIFIED has a melting point of $GET_MELTING_POINT_MODIFIED celsius and a boiling point of $GET_BOILING_POINT_MODIFIED celsius."
  fi

  fi

  # if input is element name
  if [[ $GET_INPUT =~ ^[A-Za-z]{3,}$ ]]; then
  GET_NAME=$GET_INPUT
  ATOMIC_NUMBER=$($PSQL "select atomic_number from elements where name = '$GET_INPUT'")
